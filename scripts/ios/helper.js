...
module.exports = {
    ...
    applyPluginVarsToPlists: function(pluginVariables, iosPlatform) {
        var googlePlist = plist.parse(
                fs.readFileSync(path.resolve(iosPlatform.dest), "utf8")
            ),
            appPlist = plist.parse(
                fs.readFileSync(path.resolve(iosPlatform.appPlist), "utf8")
            ),
        ...
        if (pluginVariables["SETUP_RECAPTCHA_VERIFICATION"] === "true") {
            ...
            for (i = 0; i < appPlist["CFBundleURLTypes"].length; i++) {
                if (
                    typeof appPlist["CFBundleURLTypes"][i] === "object" &&
                    appPlist["CFBundleURLTypes"][i]["CFBundleURLSchemes"]
                ) {
