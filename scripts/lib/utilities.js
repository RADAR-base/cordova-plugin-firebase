...
Utilities.parsePluginXml = function() {
    ...
    _pluginXml = Utilities.parseXmlFileToJson(
        "plugins/" + Utilities.getPluginId() + "/plugin.xml"
    );
    return _pluginXml;
};
...
Utilities.getAppName = function() {
    return Utilities.parseConfigXml()
        .widget.name._text.toString()
        .trim();
};
...
Utilities.parsePluginVariables = function() {
    ...
    if (typeof plugin.plugin.platform.length === "undefined")
        plugin.plugin.platform = [plugin.plugin.platform];
    ...
    (config.widget.plugin ? [].concat(config.widget.plugin) : []).forEach(
        function(plugin) {
            (plugin.variable ? [].concat(plugin.variable) : []).forEach(
                function(variable) {
                    if (
                        (plugin._attributes.name === Utilities.getPluginId() ||
                            plugin._attributes.id ===
                                Utilities.getPluginId()) &&
                        variable._attributes.name &&
                        variable._attributes.value
                    ) {
                        pluginVariables[variable._attributes.name] =
                            variable._attributes.value;
                    }
                }
            );
        ...
        for (const pluginId in packageJSON.cordova.plugins) {
            if (pluginId === Utilities.getPluginId()) {
                for (const varName in packageJSON.cordova.plugins[pluginId]) {
                    var varValue =
                        packageJSON.cordova.plugins[pluginId][varName];
